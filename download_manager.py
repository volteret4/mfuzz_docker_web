...
class DownloadManager:
    ...
    def execute_rsync(self, source_path: str, album_info: Dict, download_id: str, progress_callback=None) -> Dict:
        """Ejecuta rsync para copiar archivos al servidor remoto"""
        try:
            # Construir ruta relativa para el servidor remoto
            relative_path = os.path.relpath(source_path, self.local_music_root)
            remote_target_path = os.path.join(self.remote_music_path, relative_path)
            ...
            mkdir_cmd = ['ssh', self.ssh_host, f'mkdir -p "{remote_dir}"']
            
            if self.ssh_key_path and os.path.exists(self.ssh_key_path):
                mkdir_cmd.insert(1, '-i')
                mkdir_cmd.insert(2, self.ssh_key_path)
            
            logger.info(f"Creando directorio remoto: {remote_dir}")
            ...
            # Añadir trailing slash para copiar contenido del directorio
            if not source_path.endswith('/'):
                source_path += '/'
            
            rsync_cmd.extend([source_path, f'{self.ssh_host}:{remote_target_path}/'])
            
            logger.info(f"Ejecutando rsync: {' '.join(rsync_cmd)}")
            ...
            if return_code == 0:
                ...
                error_msg = '\n'.join(stderr_lines) if stderr_lines else 'Error desconocido en rsync'
    ...
    def compress_remote_album(self, remote_path: str, album_info: Dict, download_id: str) -> Dict:
        """Comprime un álbum desde el servidor remoto"""
        try:
            ...
            zip_filename = f"{safe_artist} - {safe_album} [SSH-{timestamp}].zip"
            
            # Ruta local para el ZIP
            local_zip_path = os.path.join(self.downloads_dir, zip_filename)
            
            # Crear comando para comprimir en el servidor remoto y transferir
            compress_cmd = [
                'ssh', self.ssh_host,
                f'cd "{os.path.dirname(remote_path)}" && tar -czf - "{os.path.basename(remote_path)}"'
            ]
            
            if self.ssh_key_path and os.path.exists(self.ssh_key_path):
                compress_cmd.insert(1, '-i')
                compress_cmd.insert(2, self.ssh_key_path)
            
            logger.info(f"Comprimiendo álbum remoto: {remote_path}")
    ...
    def cleanup_remote_files(self, remote_path: str) -> bool:
        """Limpia archivos temporales en el servidor remoto"""
        try:
            cleanup_cmd = ['ssh', self.ssh_host, f'rm -rf "{remote_path}"']
            
            if self.ssh_key_path and os.path.exists(self.ssh_key_path):
                cleanup_cmd.insert(1, '-i')
                cleanup_cmd.insert(2, self.ssh_key_path)
